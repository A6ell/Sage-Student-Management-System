      ...
      <AppBar style={{ backgroundColor: 'rgb(72, 42, 22)', color: 'white', padding: '8px 16px', display: 'flex', maxHeight: '100px' }} component="nav">
        <Toolbar>
          ...
          {typo === '"registrar"' && <Box sx={{ display: { xs: 'none', sm: 'block' } }}>
            {navItems.map((item) => (
              <Button style={{ backgroundColor: 'rgb(142, 83, 43)', padding: '8px 16px', marginRight: '15px' }}
                color="inherit"
                variant="text"
              ...
              >
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell align="center">Residential Address</TableCell>
                    <TableCell align="center">Date of Birth</TableCell>
                    <TableCell align="center">Starting Month</TableCell>
                    <TableCell align="center">Ending Month</TableCell>
                    <TableCell align="center">{typo === '"registrar"' && "Delete"}</TableCell>
                    <TableCell align="center">{typo === '"registrar"' && "Edit"}</TableCell>
                    {type === "student" && (
                ...
                <TableBody>
                  {data.map((row) => (
                    ...
                    >
                      ...
                      <TableCell align="center">{row.Residentioaladdress}</TableCell>
                      <TableCell align="center">{moment(row.dateofbirth).format('YYYY-MM-DD')}</TableCell>
                      <TableCell align="center">{moment(row.startingmonth).format('YYYY-MM-DD')}</TableCell>
                      <TableCell align="center">{moment(row.endingmonth).format('YYYY-MM-DD')}</TableCell>
                      <TableCell align="center">{typo === '"registrar"' && <Delete fetchTableData={fetchTableData} _id={row._id} />}</TableCell>
                      <TableCell align="center">{typo === '"registrar"' && <Update Residentioaladdress={row.Residentioaladdress} dateofbirth={row.dateofbirth} startingmonth={row.startingmonth} endingmonth={row.endingmonth}
                        fetchTableData={fetchTableData} full_name={row.full_name}
                        email={row.email} gender={row.gender} phone={row.phone}
                        program={row.program} type={row.type} _id={row._id} />}</TableCell>

                      {type === "student" && (
                        <TableCell>
                          <StudentCourses align="center" ids={row.course_ids} _id={row._id} />
                          </TableCell>
                      )}
                          {typo === '"registrar"' &&(
